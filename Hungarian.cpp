...
// * Allow for maximization by flipping matrix * -1 and + max val
...
//********************************************************//
double HungarianAlgorithm::solveAssignment(const vector<vector<double> > & DistMatrix, vector<int>& Assignment, bool maximize)
{
    if (DistMatrix.size() == 0) // check if empty
    {
        std::cerr << "DistMatrix is empty!" << endl;
        return -1;
    }

    auto tempMatrix = DistMatrix;
    if (maximize)
    {
        flipMatrix(tempMatrix);
    }


    unsigned int nRows = tempMatrix.size();
    unsigned int nCols = tempMatrix[0].size();
    // check if DistMatrix is a matrix (all rows have the same nr of elements)
    for (auto & x:DistMatrix)
    {
        if (x.size() != nCols)
        {
            std::cerr << "DistMatrix rows have unequal nr of elements! " << endl;
            return -1;
        }
    }
	...
	for (unsigned int i = 0; i < nRows; i++)
		for (unsigned int j = 0; j < nCols; j++)
            distMatrixIn[i + nRows * j] = tempMatrix[i][j];
	...
	delete[] distMatrixIn;
	delete[] assignment;
    return calcCost(DistMatrix, Assignment);
}

// Wilco: new functions to allow for maximization as well as minimization problems

double HungarianAlgorithm::calcCost(const vector<vector<double> > &DistMatrix, const vector<int>& Assignment)
{
    if (Assignment.empty() || DistMatrix.empty())
    {
        std::cerr << "Assignment matrix or DistMatrix empty!";
        return -1;
    }
    if (Assignment.size() != DistMatrix.size())
    {
        std::cerr << "Assignment matrix size different from DistMatrix !";
        return -1;
    }
    double cost = 0.0;
    for (unsigned int i = 0; i < Assignment.size(); ++i)
       cost += DistMatrix [i] [Assignment[i]];
    return cost;
}

 void HungarianAlgorithm::flipMatrix(vector <vector<double> >& DistMatrix)
  // Create maximization problem by creating a new matrix y_ij=-x_ij+maxval
 {
     if (DistMatrix.size() == 0)
         return;

      // find max val in original matrix
      double max_v = DistMatrix [0] [0];
      for (auto& n:DistMatrix)
      {
       ...
       // flip elements *-1 and + max_val
      for (auto& i:DistMatrix)
      {
          for (auto& j:i)
              j=-1.0*j + max_v;
      }
